...
<div class="card">
    <h5 class="card-header">Chỉnh sửa user</h5>
    <div class="card-body">
      <form method="post" action="{{route('users.update',$user->id)}}">
        ...
        @endphp
        <div class="form-group">
            <label for="role" class="col-form-label">Phần quyền</label>
            <select name="role" class="form-control">
                ...
                @foreach($roles as $role)
                    ...
                    <option value="sale_staff" {{(($role->role=='sale_staff') ? 'selected' : '')}}>Sale staff</option>
                    <option value="warehouse_staff" {{(($role->role=='warehouse_staff') ? 'selected' : '')}}>Warehouse staff</option>
                    <option value="shipping" {{(($role->role=='shipping') ? 'selected' : '')}}>Shipping</option>
                @endforeach
            </select>
          @error('role')
          <span class="text-danger">{{$message}}</span>
